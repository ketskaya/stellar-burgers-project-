...
describe('Редьюсер слайса constructor', () => {
  ...
  it('Должен удалить ингредиент', () => {
    ...
    const state = constructorReducer(stateWithIngredients, removeIngredient('sauce'));
    expect(state.ingredients).toHaveLength(1);
    expect(state.ingredients).not.toContainEqual(expect.objectContaining({ id: 'sauce' }));
  });
  ...
  it('Должен переместить ингредиент', () => {
    ...
    const state = constructorReducer(stateWithIngredients, moveIngredient({ from: 0, to: 1 }));
    expect(state.ingredients[0]).toEqual(expect.objectContaining({ id: 'main' }));
    expect(state.ingredients[1]).toEqual(expect.objectContaining({ id: 'sauce' }));
  });
